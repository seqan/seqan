...
.. _internal-build-manual-using-the-seqan-build-system:
...
For a more detailed view over the repository structure please read this :ref:`document <internal-infrastructure-repository-structure>`.

The SeqAn workflow is a mix between the **git-workflow** and the **forking-workflow** as described :ref:`here <internal-internal-internal-infrastructure-seqan-git-workflow>`.
...
The easiest way to install an application is described in the :ref:`getting started tutorials using linux makefiles <tutorial-getting-started-setup-seqan-linux-makefiles>`.
...
See the :ref:`internal-build-manual-using-the-find-seqan-cmake-module` for more details.
...
If you want to add an app test for your program then simply uncomment the ``seqan_add_app_test()`` call and follow the instructions in :ref:`how-to-recipes-write-app-tests` to write such an app tests.
