...
// switches on corrected heterozygote variance in allele ratios
// (higher sensitivity for hets, at the cost of more false positive snp calls)
...
bool loadGenomes(TGenomeSet & genomes,
    		 StringSet<CharString> & fileNameList,
    	         ::std::map<CharString,
               	 TGenomeSetSize> & gIdStringToIdNumMap,
    		 TGenomeNames & genomeNames)
{
    ...
    while (filecount < length(fileNameList))
    {
		...
		if(!unknownBase)
	 	{
                    std::cout << e.what() << "WARNING: Genome-File contains bases other than 'A', 'C', 'G', 'T', 'N'. All unkown bases will be considered as 'N'." << std::endl;
 		    unknownBase = true;
		}
            }
            ...
            gIdStringToIdNumMap.insert(::std::make_pair(temp, length(genomes)));
...
void
transformCoordinates(TFragmentStore & fragmentStore, const TContigPos startCoord, TOptions const &)
{
...
template <typename TFragmentStore, typename TReadCigars, typename TReadCounts,
          typename TReadClips, typename TSize, typename TContigPos, typename TOptions>
void copyNextWindowMatchesAndReads(TFragmentStore & fragmentStore,
                                   ...
                                   TOptions & options)
{
    ...
    ::std::sort(begin(fragmentStore.alignedReadStore, Standard()),
                end(fragmentStore.alignedReadStore, Standard()), LessGPos<TMatch>());

    if (options._debugLevel > 1)
        ::std::cout << "Copying matches overlapping more than one window ... \n";
    ...
    while (mIt >= mItBegin && _min((*mIt).beginPos, (*mIt).endPos)
           + (TContigPos)options.maxHitLength
           + (TContigPos)options.windowBuff >= currentWindowEnd)
    {
        if (_max((*mIt).beginPos, (*mIt).endPos) + (TContigPos)options.windowBuff > currentWindowEnd)
        {
            ...
            if (!empty(readCounts))
                appendValue(tmpReadCounts, readCounts[(*mIt).readId]);
...
//            appendValue(tmpReadClips,TPair(0,0));
            ...
            maxCoord = std::max(maxCoord, static_cast<int>std::max(mIt->beginPos, mIt->endPos));
            minCoord = std::min(minCoord, static_cast<int>std::min(mIt->beginPos, mIt->endPos));
        }
        --mIt;
    }
                      ...
                      TOptions const & options)
{
    ...
    if (options._debugLevel > 0)
        std::cout << arrayEndPos - arrayBeginPos << " matches subject to pile up correction." << std::endl;
    ...
    if (options._debugLevel > 0)
        std::cout << matchIt - matchItKeep << " matches discarded." << std::endl;
                        ...
                        TOptions const &)
{
    ...
    for (; it != itEnd; ++it)
    {
        ...
        for (unsigned i = 0; i < length(read); ++i)
            avgRQ += static_cast<int> getQualityValue(read[i]);
        // watch out, this is new: use mapping quality if given
        if ((fragmentStore.alignQualityStore[(*it).id]).score == 0 ||
            static_cast<char>(avgRQ / length(read)) < (fragmentStore.alignQualityStore[(*it).id]).score)
            (fragmentStore.alignQualityStore[(*it).id]).score = static_cast<char> (avgRQ / length(read));
    }
}
          ...
          TOptions const & options)
{
    ...
    if (length(readClips) < (arrayEndPos - arrayBeginPos))
        ::std::cout << length(readClips) << " readclips but " << (arrayEndPos - arrayBeginPos) << " many reads.\n";
    ...
    for (; it != itEnd; ++it)
    {
...
#endif
        if (m.id != m.readId)
            ::std::cout << "match id != readId \n";

        if (clipLeft + clipRight > static_cast<int>length(read) ||
	    clipLeft > static_cast<int>length(read) || 
	    clipRight > static_cast<int>length(read))
        {
            if (options._debugLevel > 0)
        ...
        if (clipLeft > 0 || clipRight > 0)
        {
            ...
            if (static_cast<int>length(read) - clipLeft - clipRight < options.minClippedLength)
            {
                if (options._debugLevel > 1)
            ...
            for (unsigned i = 0; i < length(read); ++i)
                avgRQ += static_cast<int> getQualityValue(read[i]);
            aliQ.score = static_cast<char>(avgRQ / length(read));
            //      if (extraV) ::std::cout << "aliQ.score = " << static_cast<int>aliQ.score << ::std::endl;
            ...
            aliQ.errors = static_cast<unsigned> round(static_cast<float>-score / 1000);

#ifdef SNPSTORE_DEBUG
            if (extraV)
                ::std::cout << align << std::endl;
            if (extraV)
                ::std::cout << "aliQ.errors = " << static_cast<int>aliQ.errors << std::endl;
#endif
            ...
            {
                ::std::cout << "begin bigger 300mio new beginPos = " << beginPos << " endpos=" << endPos << ::std::endl;
#ifdef SNPSTORE_DEBUG
                ::std::cout << "WARNING: clipLeft+clipRight > readLen!!??\n";
                ::std::cout << "readId = " << m.readId << "id=" << m.id << " \n";
                ::std::cout << "clipLeft = " << clipLeft << " clipRight = " << clipRight << "\n";
...
_dumpMatches(TTmpReads & reads, TTmpMatches & matches, TTmpQualities & qualities, TStr str)
{
    ...
    for (unsigned i = 0; i < length(matches); ++i)
    {
        ...
        {
            std::cout << "--" << str << "EditDist = " << static_cast<int>qualities[matches[i].id].errors << "\n";
            std::cout << "--" << str << "AvgQ     = " << static_cast<int>qualities[matches[i].id].score << "\n";
        }
...
/*      if (options.inputFormat == 1)
        ...
        if (options.laneSpecificMaxPile)
            ::std::cerr << "Lane specific:                          \tYES" << ::std::endl;
        else
            ::std::cerr << "Lane specific:                          \tNO" << ::std::endl;
    ...
    if (options.outputSNP != "")
    {
        ...
        for (unsigned i = 0; i < length(options.readFNames); ++i)
        {
            ...
            {
                readFNameNoExt[j] = static_cast<char> options.readFNames[i][j];
            }
    ...
    /////////////////////////////////////////////////////////////////////////////
    // helper variables
    Pair<int, int> zeroPair(0, 0); // TODO(serosko): check if this can be removed
    int sumreads = 0;
    int sumwindows = 0;
            ...
            if (currentWindowEnd > (TContigPos)length(genomes[i]))
	    {
            ...
            for (unsigned j = 0; j < length(options.readFNames); ++j)
            {
                ...
                addReadQualityToMatches(fragmentStore,
                                        sizeBefore,
                                        static_cast<unsigned>length(fragmentStore.alignedReadStore),
                                        options);
            ...
            TContigPos startCoord = _max(static_cast<int>options.minCoord - options.realignAddBorder, 0);
            ...
            if (!empty(fragmentStore.alignedReadStore))
            {
                ...
                //                // check for CNVs - This was probably a feature in development 
		//                   that was droppped midway. Might be usefull if someone decides
	        //                   to continue wokring on it sometime in the future.
                //                if (*options.outputCNV != 0)
                //                {
...
parseCommandLine(SNPCallingOptions<TSpec> & options, int argc, char const ** argv)
{
    ...
    addUsageLine(parser,
                 "[\\fIOPTIONS\\fP] <\\fIGENOME FILE\\fP> <\\fIALIGNMENT FILE\\fP> [<\\fIALIGNMENT FILE\\fP> ...]");
    ...
    addOption(parser, ArgParseOption("osc",
                                     "only-successful-candidates",
                                     "Output only successfully called SNP candidates."
    ...
    addOption(parser, ArgParseOption("fc",
                                     "force-call",
    ...
    addOption(parser, ArgParseOption("oa",
                                     "orientation-aware",
...
//    hideOption(parser, "cnv");
    addOption(parser, ArgParseOption("op",
                                     "output-positions",
                                     "Name of positions output file.",
                                     ArgParseArgument::STRING));
    hideOption(parser, "op");
    addOption(parser, ArgParseOption("ip",
                                     "input-positions",
    ...
    addOption(parser, ArgParseOption("eb",
                                     "exclude-border",
    ...
    addOption(parser, ArgParseOption("cq",
                                     "corrected-quality",
                                     "New quality calibration factor.",
    ...
    addOption(parser, ArgParseOption("pws",
                                     ...
                                     " (concerns memory consumption, choose smaller windows for higher coverage).",
                                     ArgParseArgument::INTEGER));
    ...
    addOption(parser, ArgParseOption("mm",
                                     "min-mutations",
    ...
    addOption(parser, ArgParseOption("pt",
                                     "perc-threshold",
    ...
    addOption(parser, ArgParseOption("mmq",
                                     "min-map-quality",
    ...
    addOption(parser, ArgParseOption("ae",
                                     "amp-efficiency",
    ...
    addOption(parser, ArgParseOption("mec",
    ...
    addOption(parser, ArgParseOption("it",
                                     "indel-threshold",
    ...
    addOption(parser, ArgParseOption("ipt",
    ...
    addOption(parser, ArgParseOption("iqt",
                                     "indel-quality-thresh",
                                     "Minimal average quality of inserted base/deletion-neighboring"
                                     " bases for indel to be called.",
                                     ArgParseArgument::INTEGER));
    ...
    addListItem(parser,
                "\\fBsnp_store\\fP \\fB-mc\\fP \\fB2\\fP \\fB-it\\fP \\fB2\\fP"
                " \\fBexampleGenome.fa\\fP \\fBexampleReads.gff\\fP \\fB-o\\fP"
                " \\fBexampleSNPs.vcf\\fP \\fB-id\\fP \\fBexampleIndels.gff\\fP",
                "Call SNPs and indels of a low-coverage example"
                " (minimum coverage and indel threshold were reduced to 2).");
    addListItem(parser,
                "\\fBsnp_store\\fP \\fB-re\\fP \\fB-mc\\fP \\fB2\\fP \\fB-it\\fP \\fB2\\fP"
                " \\fBexampleGenome.fa\\fP \\fBexampleReads.gff\\fP \\fB-o\\fP"
                " \\fBexampleSNPs.vcf\\fP \\fB-id\\fP \\fBexampleIndels.gff\\fP",
                "Computes a realignment before variant calling."
                " Now, the two 1bp insertions should have been merged into one 2bp insertion.");
...
//    getOptionValue(options.outputCNV, parser, "output-cnv");
    ...
    for (unsigned i = 0; i < countFiles; ++i)
    {
        ...
        else if (options.inputFormat != format)
        {
    ...
    //{
    //  cerr << "If mapped read file is in Eland format, a .qual or .fastq file"
    //      " containing read qualities needs to be specified." << endl << endl;
    //  return 0;
    //}
